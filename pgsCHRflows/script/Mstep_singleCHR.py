...
#modified for singleCHR chain by Harianto
...
def main(input_config):
	...
	beadRadius = 15.0
	if input_config['modeling_parameters'].has_key('beadRadius') :
		beadRadius = float( input_config['modeling_parameters']['beadRadius'] )
	
	releaseNuc = False
	if input_config['modeling_parameters'].has_key('releaseNuc') :
		releaseNuc = input_config['modeling_parameters']['releaseNuc']
	...
	newmodel = alab.modeling.singleCHRmodel(probfile,nucleusRadius=nucleusRadius,beadRadius=beadRadius,contactRange=1)
	tstart = time.time()
	...
	newmodel.set_consecutiveBeads() #3
	...
	if not useLastCoordinates:
		...
		newmodel.mdstep(500,2000)
		score = newmodel.cgstep(1000)
		newmodel.logger.debug(u"#of Intra restraints: %d and score: %.3f" %(totalIntra,score))
		print "#of Intra restraints: %d and  score: %.3f" %(totalIntra,score)
	else:
		actdist = np.genfromtxt(actdistfile,usecols=(0,1,3),dtype=(int,int,float))
		intraContactRestraints = newmodel.set_contactRestraints(actdist)
		newmodel.updateScoringFunction()
		newmodel.SimulatedAnnealing(8000000,50000,nc=5, nstep=1000) #try1
		score = newmodel.cgstep(2000)
		newmodel.logger.debug(u"score try1: %.3f" % (score))
		print "score try1: %.3f" % (score)
		newmodel.SimulatedAnnealing(500000,5000,nc=8, nstep=1000) #try2
		score = newmodel.cgstep(2000)
		newmodel.logger.debug(u"score try2: %.3f" % (score))
		print "score try2: %.3f" % (score)
		newmodel.SimulatedAnnealing(100000,1000,nc=5, nstep=1000) #try3
		score = newmodel.cgstep(1000)
		newmodel.logger.debug(u"#of Intra restraints: %d,  score: %.3f" % (len(intraContactRestraints),score))
		print "#of Intra restraints: %d  and score: %.3f" % (len(intraContactRestraints),score)
	...
	if releaseNuc:
		newmodel.removeNuc_relax(stepnum=10000)
	
	print "Simulation fb=%s finished, saved in %s.hms"%(currentfb, output_file)
	return 0
