...
#                          Nan Hua
# 
# Authors: Nan Hua; Harianto Tjong
# 
...
#Change log from v0.0.1: added locus and singleCHR modeling classes
...
__email__   = "harianto.tjong@jax.org"
...
class tadmodel(object):
    ...
    def _get_beadDistanceRestraint(self,bead1,bead2,dist,kspring=1):
        ...
        bead1,bead2 : int 
            bead id
        dist : int
            distance upper bound (surface to surface, because of IMP.core.SphereDistancePairScore)
        kspring : int
            harmonic constant k
...
class locusmodel(object):
    """
    A wrapper to do IMP modeling for a locus 
    
    Parameters
    ----------
    
    probfile : alab.matrix.contactmatrix instant
        probability matrix at 30nm-bead level, alab.matrix hdf5  format is required
    nucleusRadius : float
        radius of encapsulating sphere, default 1000(nm)
    contactRange : int
        folds for surface-surface contact coefficient
    level : loglevel, 
        default:None will record everything during caculation
        debug, info, warning, error, critical are supported
    """
    def __init__(self,probfile, beadRadius=30.0, nucleusRadius=1000, contactRange=1, level=None):
        self.probmat = alabmatrix.contactmatrix(probfile)
        self.nbead   = len(self.probmat)
        #setup log
        LEVELS={'debug':logging.DEBUG,'info':logging.INFO,'warning':logging.WARNING,'error':logging.ERROR,'critical':logging.CRITICAL}
        ...
        self.logger.setLevel(loglevel)
        #CONST
	#nucleus here is actually any sphere encapsulating locus
        self.nucleusRadius   = nucleusRadius         # nm
        #cdensity             = 107.45                # bp/nm assuming 197 bp/nucleosomes and 6 nucleosome/11 nm
        #kscale               = (0.75*15**2)**(1.0/3.0) # 3/4*r**2 where r=15nm
        #chromosome occupancy in nucleus, defined as diploid_domain_total_volume/nuclear_volume
        self.contactRange    = contactRange          # surface to surface distance scale of (r1+r2)
                                                     # for which 2 beads are considered as contact 
        #get radius of each bead; #uniform beads on chain  
        self.beadRadius = [ beadRadius for i in self.probmat.idx]
        #calculate the total volumn of DNA (diploid) and nucleus
        dnavol   = sum(np.array(self.beadRadius)**3)  #pi is crossed out
        nucvol   = self.nucleusRadius**3
        #And chromosome occupancy
        dnaocc   = dnavol / nucvol
        self.logger.debug(u'A chain consist of %d beads each %.1f nm as the modeled locus'%(self.nbead,beadRadius))
        self.logger.debug(u'Chromatin locus occupancy: %.2f inside an encapsulating sphere of %d nm'%(dnaocc,self.nucleusRadius))
        # Chromosome territory apply
        #self.genome = alabutils.genome(self.probmat.genome)
    ...
    def set_coordinates(self,coordinates=None):
        if coordinates is None:
            ...
            randomCoordinates = False
        self.xyz = np.zeros((self.nbead,3))
        self.r   = np.zeros((self.nbead,1))
        for i in range(self.nbead):
            if (randomCoordinates):
                coor = IMP.algebra.get_random_vector_in(randomCap)
            else:
    ...
    def set_contactRestraints(self,actdist,kspring=1):
        self.intraContactRestraints = self._get_contactRestraints(actdist,kspring)
        for rs in self.intraContactRestraints:#4 add intra chromosome contacts
            self.restraints.add_restraint(rs)
        return self.intraContactRestraints
    ...
    def _get_consecutiveBeadRestraints(self,lowprob=0.5, kspring=10):
        """
        calculate distance constraints to consecutive beads
        ...
        for i in range(self.nbead-1):
            b1 = i;b2 = i+1
            #calculate upper bound for consecutive domains
            #consecDist = self.beadRadius[b1]+self.beadRadius[b2]
            p = max(self.probmat.matrix[b1,b2],lowprob)
            consecDist = consecutiveDistanceByProbability(self.beadRadius[b1],self.beadRadius[b2],p,self.contactRange+1)
            # set k = 10 to be strong interaction
            rs1 = self._get_beadDistanceRestraint(b1,b2,consecDist,kspring) 
            #push restraint into list
    ...
    def _get_fmaxRestraints(self,kspring=1):
        ...
        """
        fmaxrs = []
        for i in range(self.nbead - 2):
            for j in range(i+2,self.nbead):
                if self.probmat.matrix[i,j] <= 0.999:
                    continue
                else:
                    rs1 = self._get_beadDistanceRestraint(i,j,self.contactRange*(self.beadRadius[i]+self.beadRadius[j]),kspring)
    ...
    def _get_contactRestraints(self,actdist,kspring=1):
        ...
        intracontactrs = []
        for (b1,b2,dcutoff) in actdist:
            lastdist1 = surfaceDistance(self.chain,(b1,b2)) #calc the distance between beads
            if lastdist1 <= dcutoff:
                rs1 = self._get_beadDistanceRestraint(b1,b2,self.contactRange*(self.beadRadius[b1]+self.beadRadius[b2]),kspring)
                intracontactrs.append(rs1)
                #else none contact enfored
        return intracontactrs
    #==========================end probmat restraint
    #-------------------------modeling
    def cgstep(self,step,silent=False):
        """
    ...
    def SimulatedAnnealing(self,hot,cold,nc=10,nstep=500):
        ...
        for i in range(nc):
            t = hot-dt*i
            s = self.mdstep(t,nstep,silent=True)
            self.logger.info(u"      Temp=%d Step=%d Time=%.1fs Score = %.2f"%(t,nstep,alabutils.timespend(t0),s))
            print "      Temp=%d Step=%d Time=%.1fs Score = %.2f"%(t,nstep,alabutils.timespend(t0),s)
        s= self.mdstep(cold,nstep,silent=True)
        self.logger.info(u"      Temp=%d Step=%d Time=%.1fs Score = %.2f"%(cold,nstep,alabutils.timespend(t0),s))
        print "      Temp=%d Step=%d Time=%.1fs Score = %.2f"%(cold,nstep,alabutils.timespend(t0),s)
        self.cgstep(100,silent=True)
    ...
    def SimulatedAnnealing_Scored(self,hot,cold,nc=10,nstep=500,lowscore=10):
        ...
        for i in range(nc):
            ...
            self.mdstep(t,nstep,silent=True)
            score = self.cgstep(100,silent=True)
            self.logger.info(u"      Temp=%s Step=%s Time=%.1fs Score=%.2f"%(t,nstep,alabutils.timespend(t0),score))
            print "      Temp=%s Step=%s Time=%.1fs Score=%.2f"%(t,nstep,alabutils.timespend(t0),score)
            if score < lowscore:
                return t,score
                break
        self.mdstep(cold,300,silent=True)
        score = self.cgstep(100,silent=True)
        self.logger.info(u"      Temp=%s Step=%s Time=%.1fs Score=%.2f"%(cold,nstep,alabutils.timespend(t0),score))
        print "      Temp=%s Step=%s Time=%.1fs Score=%.2f"%(cold,nstep,alabutils.timespend(t0),score)
        return t,score
    ...
    def shrinkingOptimization(self,drange,shrinkScore,minscore,interScale):
        ...
        for r_nuc in nucrads:
            ...
            temp, score = self.SimulatedAnnealing_Scored(2000,300,nc=2, lowscore=shrinkScore)
            score = self.cgstep(500,silent=True)
            self.logger.debug(u"Score optimizing temporary NE %dmm done: %.4f"%(r_nuc,score))
            print "Score optimizing temporary NE %dmm done: %.8f"%(r_nuc,score)
            if score > minscore:
                if expanded == False: #haven't done expansion steps before
                    self.logger.debug(u"\t    --- Start expanding ---")
                    ...
                    self.logger.debug(u'\t     Score is still high after expansion: %.4f' %(score))
                    print '\t     Score is still high after expansion: %.4f' %(score)
                    break
            #-
        #--
        # Putting back original radius cap  #
        ...
        temp, score = self.SimulatedAnnealing_Scored(2000,300, nc=2, nstep=1000, lowscore=1)
        self.logger.debug(u"Recover nucleus %.1f nm at T=%.1f K, score: %.8f"%(self.nucleusRadius,temp,score))
        print "Recover nucleus %.1f nm at T=%.1f K, score: %.3f"%(self.nucleusRadius,temp,score)
        return 0

    def removeNuc_relax(self, stepnum=5000):
        self.logger.debug(u"Removing engulfing sphere")
        print "Remove NE and relax..."
        self.restraints.remove_restraint(self.nucleusEnvelopeRestraint)
        self.updateScoringFunction()
        self.SimulatedAnnealing(300,290, nc=2, nstep=stepnum)
        temp, score = self.SimulatedAnnealing_Scored(290,273, nc=2, nstep=stepnum, lowscore=1)
        self.logger.debug(u"Relaxing structure T=%.1f K, score: %.1f"%(temp,score))
        return 0

    #==================utils
    def evaluateRestraints(self,restraintset,tolerance=1.0):
        #locus resolution can have tolerance of 1.0
        total = 0
        for rs in restraintset:
            score = rs.get_score()
            rsstr = rs.get_name()
...
class singleCHRmodel(object):
    """
    A wrapper to do IMP modeling for a single chromosome
    ...
    def __init__(self,probfile, beadRadius=30.0, nucleusRadius=5000, contactRange=1, chromosomeOccupancy=0.2, level=None):
        self.probmat = alabmatrix.contactmatrix(probfile)
        self.nbead   = len(self.probmat)
        #setup log
        LEVELS={'debug':logging.DEBUG,'info':logging.INFO,'warning':logging.WARNING,'error':logging.ERROR,'critical':logging.CRITICAL}
        ...
        self.genome          = alabutils.genome(self.probmat.genome)
        rho                  = self.occupancy*self.nucleusRadius**3/(2*sum(self.genome.info['length']))
        #self.beadRadius = [ beadRadius for i in self.probmat.idx] #this is for a constant radius
        self.beadRadius = [(rho * (index['end'] - index['start'])) ** (1.0/3.0) for index in self.probmat.idx]
        #calculate the total volumn of DNA (diploid) and nucleus
        dnavol   = sum(np.array(self.beadRadius)**3)  #pi is crossed out
        nucvol   = self.nucleusRadius**3
        #And chromosome occupancy
        dnaocc   = dnavol / nucvol
        self.logger.debug(u'A chain consist of %d beads each %.1f nm as the modeled chromosome'%(self.nbead,beadRadius))
        self.logger.debug(u'Chromatin locus occupancy: %.2f inside an encapsulating sphere of %d nm'%(dnaocc,self.nucleusRadius))
        # Chromosome territory apply
        #self.genome = alabutils.genome(self.probmat.genome)
    ...
    def set_consecutiveBeads(self):
        # Setup consecutive bead upper bound constraints
        self.consecutiveBeadRestraints = self._get_consecutiveBeadRestraints(kspring=10)
        for rs in self.consecutiveBeadRestraints: #3
            self.restraints.add_restraint(rs)
        self.logger.debug(u"Total consecutive bead restraints %d"%(len(self.consecutiveBeadRestraints)))
        print "Total consecutive bead restraints %d"%(len(self.consecutiveBeadRestraints))
    ...
    def _get_consecutiveBeadRestraints(self,kspring=10):
        """
        calculate distance constraints to consecutive beads
    ...
    #==================utils
    def evaluateRestraints(self,restraintset,tolerance=1.0):
    #tolerance of high resol beads (e.g. < 100 nm) can be set as 1 particle radius
        total = 0
        for rs in restraintset:
            score = rs.get_score()
            rsstr = rs.get_name()
