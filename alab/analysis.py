...
# 
# Authors: Nan Hua
# Other contributor: Harianto Tjong
...
class structuresummary(object):
    ...
    def plotRadialPosition(self,figurename,chrom,format='pdf',color='dodgerblue',nucleusRadius=5000.0, vline=None):
        """
...
class locusStructuresummary(object):
    """
    This class offers a series of methods to study the model structure populations.
    Special for locus modeling
    ...
    def __init__(self,target,usegrp=None,nstruct=10000,pid=10,**kwargs):
        if os.path.isdir(target):
            ...
            if usegrp==None:
                raise RuntimeError, "group key is not specified!"
            firststr    = files.locusStructures(os.path.join(target,"copy0.hms"),[usegrp])
            self.idx    = firststr.idx
            self.genome = firststr.genome
            self.usegrp = usegrp
            self.nbead  = len(self.idx)
    ...
    def _readStructures(self,target,pid,silence=True):
        ...
        """
        import multiprocessing
        coor_shared       = multiprocessing.Array('d',self.nstruct*self.nbead*3) #10000 * nbead* 3
        consecVio_shared  = multiprocessing.Array('d',self.nstruct)
        contactVio_shared = multiprocessing.Array('d',self.nstruct)
        score_shared      = multiprocessing.Array('d',self.nstruct)
        intrares_shared   = multiprocessing.Array('d',self.nstruct)
        ...
        for process in readpool:
            process.join()
        self.coordinates = np.frombuffer(coor_shared.get_obj()).reshape((self.nstruct,self.nbead,3))
        self.score       = np.frombuffer(score_shared.get_obj())
        self.consecutiveViolations = np.frombuffer(consecVio_shared.get_obj())
        self.contactViolations     = np.frombuffer(contactVio_shared.get_obj())
        self.intraRestraints = np.frombuffer(intrares_shared.get_obj())
    ...
    def _readInfo(self,target,start,end,usegrp,
                  ...
                  consecVio_shared,contactVio_shared,
                  score_shared,
                  intrares_shared,silence=True):
        """
        Read in all information
        """
        arr = np.frombuffer(coor_shared.get_obj())
        modelcoor = arr.reshape((self.nstruct,self.nbead,3)) #10000 * nbead* 3
        for i in range(start,end):
            try:
                if not silence:
                    print(i)
                sts = files.locusStructures(os.path.join(target,'copy'+str(i)+'.hms'),[usegrp])
    ...
    def getContactMap(self,contactRange=1):
        ...
        modelmap.matrix = cmap[:,:]
        modelmap.matrix = modelmap.matrix/self.nstruct
    ...
    def getAveragePairwiseDistance(self,form='list'):
        ...
        form : str
            ...
            'matrix' return the matrix form
        """
        pdistMean = np.zeros((self.nbead,self.nbead))
        ...
        if form == 'list':
            return pdistMean[np.triu_indices(self.nbead,1)]
        else:
            return pdistMean
    ...
    def getRadGyration(self, structs='all'):
        """
        Calculate radius of gyration for every structure in the summary (hss)
        ...
        Returns
        -------
        N*1 vector : radius of gyration for the locus in all structures in population
        It also calculates the center of mass (x,y,z), thus radial position can be returned as well.
        """
        allrg = []
        if structs == 'all':
            structs = range(self.nstruct)
        for i in np.array(structs):
            mass = [l**3 for l in self.radius.flatten()]
            totm = sum(mass)
            xm1 = [(m1*i1,m1*j1,m1*k1) for (i1,j1,k1), m1 in izip(self.coordinates[i,:,:],mass)]
            comxyz = [sum(u)/totm for u in izip(*xm1)] #center of mass coordinates
            mma = sum( u**2 for u in comxyz ) #square root of this is the radial position
            rra= sum(mx*xi+my*yi+mz*zi for (xi,yi,zi),(mx,my,mz) in izip(self.coordinates[i,:,:],xm1))
            rg = np.sqrt(rra/totm - mma)
            allrg.append(rg)
        return np.array(allrg)

    def getRMSD(self, structs=None, blist=None):
        """
        Calculate Root mean-squared displacements for a given set of beads in the summary (hss)

        Parameters
        ----------
        structs : array-like
            list of all structures to calculate (ordered based on hss)
        blist : array-like
            list of all beads to calculate
        Returns
        -------
        (nb-1)*ns matrix : displacement between all vs all beads; ns=len(structs), nb=len(beads)
        """
        if structs is None:
            structs = range(self.nstruct)
        if blist is None:
            blist = range(self.nbead)
        nb = len(blist) #matrix dimension
        ns = len(structs) #matrix dimension
        rmsd = np.empty((nb-1,0), float)
        ldist = np.array([abs(k-j) for j,k in combinations(range(nb),2)])#linear distances in bead unit
        for i in np.array(structs):
            x = self.coordinates[i,blist,] #coordinates of a group of beads in the i-th structure
            pairdist_sq = pdist(x)**2 #distance-squared list
            d = [np.sqrt(pairdist_sq[np.flatnonzero(ldist==j)].mean()) for j in range(1,nb)]
            rmsd = np.column_stack([rmsd,d])
        return rmsd
    ...
    def getChromosomeRadialPosition(self,chrom,nucleusRadius=5000.0):
        ...
        Returns
        -------
        N*1 vector : radial position for the chromosome in all structures in population
        """
        
        Aids = np.flatnonzero(self.idx['chrom'] == chrom)
        
        radial = np.zeros(self.nstruct)
        for i in range(self.nstruct):
            xyzA = self.coordinates[i][Aids]
            comA = utils.centerOfMass(xyzA,self.radius[Aids])
            radial[i]   = np.linalg.norm(comA)/nucleusRadius
        #-
        return radial
    ...
    def plotRadialPosition(self,figurename,chrom,format='pdf',color='dodgerblue',nucleusRadius=5000.0):
        ...
        Aids = np.flatnonzero(self.idx['chrom'] == chrom)
        #calculate average radial position
        radial = self.getBeadRadialPosition(Aids,nucleusRadius).mean(axis=1)
        #calculate midpos
        mega = 1.e-6
        midpos = mega*(self.idx[Aids]['end'] + self.idx[Aids]['start'])/2
